...
class Question {
    constructor() {
        ...
        else this.correct = Math.round((a / b) * 100) / 100;
...
function displayResult() {
    ...
    result_field.innerHTML = `Ви дали ${points} правильних відповідей з ${total_question_count}.<br>Точність: ${accuracy}%`;
...
difficulty_btn[0].addEventListener("click", () => setDifficulty(15));
difficulty_btn[1].addEventListener("click", () => setDifficulty(20));
difficulty_btn[2].addEventListener("click", () => setDifficulty(25));
...
for (let i = 0; i < answer_buttons.length; i++) {
    answer_buttons[i].addEventListener("click", function () {
        ...
        if (answer_buttons[i].innerHTML == current_question.correct) {
            points += 1;
            answer_buttons[i].style.background = '#388E3C'; // зелений
        } else {
            answer_buttons[i].style.background = '#F44336'; // червоний
        }

        setTimeout(() => {
            answer_buttons[i].style.background = "#222"; // темно-сірий фон
            answer_buttons[i].style.border = "2px solid #f44336";
            answer_buttons[i].style.outline = "1px solid #f44336";
            answer_buttons[i].style.outlineColor = "#f44336";
